<!--  -->
<template>
	<unicloud-db ref="udb" v-slot:default="{ data, loading, error, options }" collection="questions" :page-size="10">
		<swiper :current="current" class="swiper" @change="swiperChange">
			<swiper-item v-for="(each, swiperIndex) in data">
				<view class="content">{{ each.content }}</view>
				...
				<view style="margin-top: 20rpx;" class="result" v-if="each.result && each.result.split('').some(r => !each.answer.includes(r))">
...
export default {
	...
	setup() {
		...
		function swiperChange({ detail }) {
			...
			if (detail.current == udb.value.dataList.length - 3) {
				udb.value.loadMore();
			}
		}
